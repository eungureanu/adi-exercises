displayController();
console.log("#1 Numele tarilor participante:", participatingCountries());
console.log(countriesByPopulation());
console.log("#2.A TOP 5 tari cu populatia cea mai numeroasa:", topFiveExistingCountriesByPopulation());
console.log("#2.B TOP 5 tari cu populatia cea mai numeroasa (inclusiv tari care NU mai exista):", topFiveCountriesByPopulation());
console.log("#3 Tarile care inca exista si incep cu litera A:", countriesLetterA());
...
function nationCode(){
  const arr = getOlympicData();
  const newArr = arr.map(({Nation, Code}) => ({nation: Nation, code: Code})); //arrow function for 
  // const newArr = arr.map(function(item) {
  //   const { Nation, Code } = item;
  //   return { nation: Nation, code: Code };
  // });  
  return newArr;
}

function maxApps(){
  // just getting the max Apps value
  // const arr = getOlympicData().map(item => item.Apps)
  // return Math.max(...arr);

  // ordering by Apps high to low and displaying the first from the list
  // const arr = getOlympicData();
  // const arrApp = arr.map((item) => [item.Nation, item.Apps]);
  // const orderedArrayPop = arrApp.sort((a,b) => b[1]-a[1]); //population is ordered high to low
  // return orderedArrayPop[0];
...
function displayController() {
  ...
  const countryCode = document.getElementById("nation-code");
  for (let i = 0; i < nationCode().length; i++) {
    const listItem = document.createElement("li");
    listItem.innerHTML = nationCode()[i].nation+" "+nationCode()[i].code;
    countryCode.appendChild(listItem);
  }
